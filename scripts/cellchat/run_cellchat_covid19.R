# Build CellChat objects for the COVID-19 moderate-versus-severe comparison.
#
# Input directories are produced by notebooks/01_global_atlas_analysis.ipynb:
#   outputs/cellchat/covid19/input/<group>/
#
...
input_dir <- Sys.getenv(
  "CELLCHAT_COVID19_INPUT_DIR",
  unset = file.path("outputs", "cellchat", "covid19", "input")
)
...
groups_to_run <- c("COVID19_pro_Moderate", "COVID19_pro_Severe")
...
for (group_name in groups_to_run) {
  message("\nProcessing COVID-19 CellChat group: ", group_name)
  tryCatch(
...
message("All COVID-19 CellChat groups completed.")
