# Build CellChat objects for the CAR-T CRS comparison.
#
# Input directories are produced by notebooks/01_global_atlas_analysis.ipynb:
#   outputs/cellchat/cart/input/<group>/
# Each group directory must contain expression_matrix.mtx, genes.csv, and meta.csv.
#
...
input_dir <- Sys.getenv(
  "CELLCHAT_CAR_T_INPUT_DIR",
  unset = file.path("outputs", "cellchat", "cart", "input")
)
...
groups_to_run <- c("CAR-T_CRS_before", "CAR-T_CRS_pro", "CAR-T_CRS_con")
workers <- as.integer(Sys.getenv("CELLCHAT_WORKERS", unset = "8"))
...
for (group_name in groups_to_run) {
  message("\nProcessing CAR-T CellChat group: ", group_name)
  tryCatch(
...
message("All CAR-T CellChat groups completed.")
