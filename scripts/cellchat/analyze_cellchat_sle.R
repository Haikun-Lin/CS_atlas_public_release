...
# ============================================================
# Fig. 5H Supporting Data Values
# Severe-SLE monocyte outgoing signaling
#
# Uses the exact `plot_data` object that was used to generate
# the frozen Fig. 5H.
#
# Output columns:
# Pathway_order | Pathway |
# Source_group_order | Source_group |
# Total_communication_probability |
# Relative_signaling_strength
# ============================================================
...
output_file <- "Fig5H_supporting_data_values.xlsx"
...
# ------------------------------------------------------------
# 1. Check that the plotting dataframe exists
# ------------------------------------------------------------

if (!exists("plot_data")) {
  stop(
    "Object 'plot_data' was not found. ",
    "Run the original Fig. 5H plotting code first."
  )
}


# ------------------------------------------------------------
# 2. Build clean SDV table directly from plotted values
# ------------------------------------------------------------
...
fig5h_sdv <- plot_data %>%
  ...
  transmute(
    ...
    Source_group_order,
    Source_group,
    Total_communication_probability = total_prob,
    Relative_signaling_strength = relative_strength
  ) %>%
...
check_df <- fig5h_sdv %>%
  ...
  summarise(
    Relative_strength_sum =
      sum(Relative_signaling_strength),
    .groups = "drop"
  )
...
if (
  any(
    abs(
      check_df$Relative_strength_sum - 1
    ) > 1e-10
  )
) {
  stop(
    "Relative signaling strengths do not sum to 1 ",
    "within every pathway."
  )
}
